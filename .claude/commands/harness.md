...
```json
{
  ...
  "steps": [
    ...
    { "step": 4, "name": "qa-api", "status": "pending", "agent": "qa/api-tester" }
  ]
}
```
...
- `steps[].agent`: (선택) `"qa/playwright"` 형식으로 지정. `agents/{domain}/_base.md` + `agents/{agent}.md` 두 파일을 합쳐 step 프롬프트 앞에 주입한다. 생략하면 범용 개발자 역할로 실행된다.
