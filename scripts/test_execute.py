...
def tmp_project(tmp_path):
    """phases/, CLAUDE.md, .claude/CLAUDE.md, docs/ 를 갖춘 임시 프로젝트 구조."""
    ...
    claude_dir.mkdir()
    (claude_dir / "CLAUDE.md").write_text("# Project Rules\n- CRITICAL: project specific rule", encoding="utf-8")
...
class TestLoadGuardrails:
    ...
    def test_loads_project_claude_md(self, executor, tmp_project):
        with patch.object(ex, "ROOT", tmp_project):
            result = executor._load_guardrails()
        assert "CRITICAL: project specific rule" in result
    ...
    def test_no_project_claude_md(self, executor, tmp_project):
        import shutil
        shutil.rmtree(tmp_project / ".claude")
        with patch.object(ex, "ROOT", tmp_project):
            result = executor._load_guardrails()
        assert "CRITICAL: project specific rule" not in result
        assert "# Rules" in result
...
class TestLoadAgent:
    def _make_agents(self, tmp_project, base_content=None, agent_content=None):
        agents_dir = tmp_project / "agents" / "qa"
        agents_dir.mkdir(parents=True, exist_ok=True)
        if base_content is not None:
            (agents_dir / "_base.md").write_text(base_content)
        if agent_content is not None:
            (agents_dir / "playwright.md").write_text(agent_content)

    def test_no_agent_field_returns_empty(self, executor, tmp_project):
        with patch.object(ex, "ROOT", tmp_project):
            result = executor._load_agent({})
        assert result == ""

    def test_empty_agent_field_returns_empty(self, executor, tmp_project):
        with patch.object(ex, "ROOT", tmp_project):
            result = executor._load_agent({"agent": ""})
        assert result == ""

    def test_agent_file_only(self, executor, tmp_project):
        self._make_agents(tmp_project, agent_content="# Playwright Agent")
        with patch.object(ex, "ROOT", tmp_project):
    ...
    def test_base_comes_before_agent(self, executor, tmp_project):
        self._make_agents(tmp_project, base_content="BASE_CONTENT", agent_content="AGENT_CONTENT")
        with patch.object(ex, "ROOT", tmp_project):
            result = executor._load_agent({"agent": "qa/playwright"})
        assert result.index("BASE_CONTENT") < result.index("AGENT_CONTENT")
